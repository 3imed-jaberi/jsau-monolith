...
import { VIEW_PATH } from '../middlewares/view-engine.middleware.js'
...
export async function getPosts(request, response) {
  ...
  const viewPath = path.join(VIEW_PATH, 'home.njk')
  response.render(viewPath, { items: allPosts })
}
...
export async function getPostById(request, response) {
  ...
  const viewPath = path.join(VIEW_PATH, 'single-post.njk')
  response.render(viewPath, { item: post })
}
